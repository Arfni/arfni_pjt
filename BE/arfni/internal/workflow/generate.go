...
func GenerateDockerCompose(s *stack.Stack, projectDir string) (string, error) {
	...
	for name, service := range s.Services {
		...
		if service.Spec.Image != "" {
			...
			// Dockerfile path if specified
			if service.Spec.Dockerfile != "" {
				dcService.Build.Dockerfile = service.Spec.Dockerfile
			} else {
				dcService.Build.Dockerfile = "Dockerfile"
			}
		}
...
func WriteDockerCompose(s *stack.Stack, projectDir string) error {
	...
	// Create compose directory (.arfni/compose)
