            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="text-sm text-gray-600 p-4 rounded-lg border border-gray-300 flex gap-4 items-center">
                <div className="flex-1">
                  <p className="font-semibold mb-2">{t('ai.analysisTitle')}</p>
                  <ul className="list-disc list-inside space-y-1">
                    <li>{t('ai.analysisList.serviceConfig')}</li>
                    <li>{t('ai.analysisList.memoryRequirement')}</li>
                    <li>{t('ai.analysisList.stability')}</li>
                    <li>{t('ai.analysisList.recommendation')}</li>
                    <li>{t('ai.analysisList.costCalculation')}</li>
                  </ul>
                </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <h4 className="font-semibold text-blue-900 mb-2">
                      {t('ai.recommended')} {result.recommended_tier.tier_name}
                    </h4>
                    ...
                    {result.recommended_tier.details?.ec2_items?.[0]?.description && (
                      <div className="mb-2 p-2 bg-white rounded border border-blue-200">
                        <p className="text-xs text-gray-800 leading-relaxed">
                          <span className="font-semibold text-blue-800">{t('ai.selectedReason')}</span> {result.recommended_tier.details.ec2_items[0].description}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="text-xs text-gray-700 space-y-2 bg-white p-2 rounded border border-blue-100">
                      <div className="font-semibold text-blue-900">{t('ai.monthlyCost')}</div>
                      {result.recommended_tier.details?.ec2_items?.map((item, idx) => (
                        <div key={idx} className="pl-2">
                          • {item.instance_type} {t('ai.instance')} ${item.total_price.toFixed(2)}
                        </div>
                      ))}
                      {result.recommended_tier.details?.storage_items?.filter(item => item.name.includes('Storage')).map((item, idx) => (
                        <div key={idx} className="pl-2">
                          • {t('ai.ebsStorage')} {item.count}GB: ${item.total_price.toFixed(2)}
                        </div>
                      ))}
                      {result.recommended_tier.data_transfer_cost > 0 && (
                        <div className="pl-2">
                          • {t('ai.dataTransfer')} ${result.recommended_tier.data_transfer_cost.toFixed(2)}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                    <h4 className="font-semibold text-gray-800 mb-1 text-sm">
                      {t('ai.budgetOption')} {result.budget_tier.tier_name}
                    </h4>
                    ...
                    {result.budget_tier.details?.ec2_items?.[0]?.description && (
                      <div className="mb-1 p-2 bg-white rounded border border-gray-300">
                        <p className="text-xs text-gray-700 leading-relaxed">
                          <span className="font-semibold">{t('ai.selectedReason')}</span> {result.budget_tier.details.ec2_items[0].description}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="text-xs text-gray-600 space-y-1 bg-white p-2 rounded border border-gray-200">
                      <div className="font-semibold">{t('ai.monthlyCost')}</div>
                      {result.budget_tier.details?.ec2_items?.map((item, idx) => (
                        <div key={idx} className="pl-2">
                          • {item.instance_type} {t('ai.instanceShort')} ${item.total_price.toFixed(2)}
                        </div>
                      ))}
                      ...
                      {result.budget_tier.data_transfer_cost > 0 && (
                        <div className="pl-2">• {t('ai.dataTransfer')} ${result.budget_tier.data_transfer_cost.toFixed(2)}</div>
                      )}
                    </div>
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                    <h4 className="font-semibold text-gray-800 mb-1 text-sm">
                      {t('ai.performanceOption')} {result.performance_tier.tier_name}
                    </h4>
                    <div className="mb-1 px-2 py-1 bg-gray-200 rounded inline-block">
                    ...
                    {result.performance_tier.details?.ec2_items?.[0]?.description && (
                      <div className="mb-1 p-2 bg-white rounded border border-gray-300">
                        <p className="text-xs text-gray-700 leading-relaxed">
                          <span className="font-semibold">{t('ai.selectedReason')}</span> {result.performance_tier.details.ec2_items[0].description}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="text-xs text-gray-600 space-y-1 bg-white p-2 rounded border border-gray-200">
                      ...
                      {result.performance_tier.data_transfer_cost > 0 && (
                        <div className="pl-2">• {t('ai.dataTransfer')} ${result.performance_tier.data_transfer_cost.toFixed(2)}</div>
                      )}
                    </div>
                  </div>
                  ...
                  {result.optimization_tips.length > 0 && (
                    <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                      <h4 className="font-semibold text-green-900 mb-2 text-sm">{t('ai.tips')}</h4>
