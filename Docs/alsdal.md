# 2025.10.22
- 테스트 버전 UI 빌드 및 오류 수정
- 러스트 커맨드 연결 방법 학습
- 캔버스 리소스 속성 필드 조사
- UI 레퍼런스 조사

# 2025.10.23
- EC2 연결 검증 확인 커맨드 연결
- 프로젝트 생성 모달 EC2 연결 실패 단계 추가
- 원격 서버 연결 시 사용자명 입력 받도록 수정
- 모달 컴포넌트 단계별로 분리

# 2025.10.24
- 테스트 앱 흐름 설계 및 구체화

# 2025-10-27
- 테스트 버전 화면 동작 흐름 수정(프로젝트 생성 및 연결 - 프로젝트 목록 - 캔버스/로그)
- 로컬 도커 설치, 실행 여부 검증 기능 구현
- 로컬 프로젝트 목록 불러오기 기능 구현
- 원격 서버 목록 조회, 저장, 삭제 기능 구현

# 2025.10.29
- 프로젝트 목록 디자인
- 프로젝트 목록 화면 리팩토링

# 2025.10.30
- 프로젝트 목록 컴포넌트 분리 및 리팩토링
- 프로젝트 목록, 서버 정보, 프로젝트 생성 모달 UI 수정
- 프로젝트 로그 화면 터미널 추가
- 프로젝트 로그 화면 원격 서버 정보 연동

# 2025.10.31
- 서버 추가할 때 창 안닫히게 수정
- 서버 추가 창 닫혀도 정보 초기화되게 수정
- 서버 정보 편집 기능 추가
- 서버 편집 아이콘 배경 UI 수정
- 서버 추가/편집, 프로젝트 생성할 때 단계 누락되면 에러 표시
- 서버 선택,추가 모달에서 바깥 배경 클릭시 모달 안닫히게 수정
- 로컬/EC2 토글할 때 프로젝트 화면 미세하게 움직임 수정
- 새 프로젝트 생성 폴더 버튼 UI 수정
- 로딩 문구, 빈 캔버스 문구 영어로 수정
- 로그 화면 docker ps 커맨드 연결

# 2025.11.03
- 로그 화면 컨테이너 목록 표시
- 로그 색 구분
- 로그 화면 도커 커맨드 버튼 수정
- 로그 화면 도커 커맨드 연결
- 설정 화면 구현

# 2025.11.04
- 프로젝트 목록에서만 삭제 시 DB의 데이터도 함께 제거하도록 수정
- 새 프로젝트 생성 시 같은 이름의 폴더 남아있다면 사용자에게 오류 문구 표시
- 서버 선택하지 않은 상태에서 새로고침 시 프로젝트 목록이 조회되는 오류 수정
- 프로젝트 폴더가 삭제되거나 이동된 상태에서 프로젝트 열었을 시 경고 메세지 띄운 후 목록에서 제거 유도창 띄우기

# 2025.11.05
- 배포 화면 UI 수정
- 배포 화면 로컬/EC2 프로젝트 상태에 맞게 UX 분기
- 배포 완료/실패 모달 수정
- 컨테이너 빌드 실시간 상태 표시 구현

# 2025.11.06
- 설정 화면 open API 키 저장/삭제 기능 연결
- 상단바 제작
- 로그 페이지 헤더 수정
- 로그 페이지 왼편 사이드바 구현
- 서버 최적화 분석 페이지 구현

# 2025.11.07
- 로그 페이지 사이드바 재구현
- 서버 최적화 분석 기능 연결
- 모니터링 로그 로딩 페이지 뒤로가기 버튼 추가
- 로그 페이지 컴포넌트 분리

# 2025.11.10
- 배포 화면 재구현
	- 배포 화면 UI 수정
	- 배포 진행 상태 동기화하여 표시
	- 배포 중지 기능 구현
	- 배포 상태 구분하여 제목 수정
	- 배포할 컨테이너 목록 빌드 성공/실패 여부 제공
- 배포 화면 컴포넌트 분리


# 2025.11.11
- 프로젝트 삭제 안되는 버그 수정
- 로그페이지 디폴트 탭 수정
- 로그페이지 터미널탭 Open Tunnel 버튼 제거
- 모니터링 화면 새창 보기 버튼 연결
- 배포 중 컨테이너 목록 오류 수정
- 로그 페이지 탭 동작 방식 수정
- 서버 최적화 화면 UI 수정
- 모니터링 탭 화면 추가
- 프로젝트 문자열 스트링 변환
- 한국어 번역
- 설정 화면 컴포넌트 분리
- 설정 화면 API 키 화면 구현
- 배포 화면 크기 조정
- 배포 완료 모달 텍스트 배치 수정
- 로그 페이지 컨테이너 삭제 오류 수정

# 2025.11.12
- pretendard 폰트 적용
- 새 창에서 열기 오류 수정
- 프로젝트 홈 UI 수정
	- 프로젝트 화면 사이드바 버튼 크기 키우기
	- 메뉴 버튼 상단 패딩 추가
	- 프로젝트 카드 제목 크기 키움
	- 로컬/EC2/플러그인 프로젝트 본문 패딩 통일
- api 키 활성화 중복 오류 수정
- EC2 문구 Remote로 대체

# 2025.11.13
- 싸피마켓 거래 완료 기능 구현
- 푸터, 게시글, 회원 가입, 마이페이지 UI/UX 수정
- 뒤로가기 스택 로직 수정
- 팀페이지 구현